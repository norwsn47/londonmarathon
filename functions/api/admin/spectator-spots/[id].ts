...
interface SSRow {
  ...
  nearest_stations: string | string[]; crowd_notes: string; url: string; maps_url: string; sort_order: number;
}
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  if (request.method === 'PUT') {
    ...
    const { name, description, distance_km, distance_mile, lat, lng, crowd_notes, url, maps_url, sort_order } = body;
    ...
    const result = await env.DB.prepare(
      `UPDATE spectator_spots SET name=?, description=?, distance_km=?, distance_mile=?, lat=?, lng=?,
       nearest_stations=?, crowd_notes=?, url=?, maps_url=?, sort_order=?, updated_at=datetime('now') WHERE id=?`
    ).bind(name.trim(), (description ?? '').trim(), distance_km, distMile, lat, lng, stations, (crowd_notes ?? '').trim(), (url ?? '').trim(), (maps_url ?? '').trim(), sort_order ?? 0, id).run();
