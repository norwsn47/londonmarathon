...
interface SSRow {
  ...
  nearest_stations: string; crowd_notes: string;
...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (request.method === 'GET') {
    const { results } = await env.DB.prepare(
      'SELECT id, name, description, distance_km, distance_mile, lat, lng, nearest_stations, crowd_notes, sort_order, updated_at FROM spectator_spots ORDER BY sort_order ASC'
    ).all<SSRow>();
  ...
  if (request.method === 'POST') {
    ...
    const { name, description = '', distance_km, distance_mile, lat, lng, crowd_notes = '', sort_order = 0 } = body;
    ...
    await env.DB.prepare(
      'INSERT INTO spectator_spots (id, name, description, distance_km, distance_mile, lat, lng, nearest_stations, crowd_notes, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).bind(id, name.trim(), description.trim(), distance_km, distMile, lat, lng, stations, crowd_notes.trim(), sort_order).run();
